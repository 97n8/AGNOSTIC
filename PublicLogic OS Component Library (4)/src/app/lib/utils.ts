/**
 * DOM and General Utilities for Phillipston Governance OS
 */
...
export function el(tag: string, attrs: any = {}, children: any = []) {
  ...
  for (const [k, v] of Object.entries(attrs || {})) {
    if (k === "class") node.className = v as string;
      ...
      node.addEventListener(k.slice(2).toLowerCase(), v as any);
...
export function qs(sel: string, root: ParentNode = document) {
  return root.querySelector(sel);
}

export function qsa(sel: string, root: ParentNode = document) {
